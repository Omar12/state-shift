...
export default function OnboardingScreen({ onComplete }: Props) {
  ...
  return (
    <div className="flex flex-col flex-1 justify-between px-5 py-8">
      ...
      <div className="flex flex-col gap-6 mt-12">
        <h1 className="font-serif text-3xl font-semibold text-stone-800 tracking-tight leading-snug text-balance">
          {current.title}
        </h1>
        <p className="text-base text-stone-600 leading-relaxed text-pretty">{current.body}</p>
        {current.disclaimer && (
          <p className="text-sm text-stone-500 leading-relaxed bg-stone-50 rounded-lg px-3 py-2.5">
            {current.disclaimer}
          </p>
        )}
      </div>
