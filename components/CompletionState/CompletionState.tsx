"use client";

type Props = {
  allUsed?: boolean;
  nothingHelped?: boolean;
  onHome: () => void;
};

export default function CompletionState({ allUsed, nothingHelped, onHome }: Props) {
  return (
    <div className="flex flex-col flex-1 px-5 py-8 gap-6 justify-center">
      <div className="flex flex-col gap-4">
        <div className="w-14 h-14 rounded-2xl flex items-center justify-center animate-pop-in animation-delay-200" style={{ backgroundColor: "var(--color-settle-green)" }}>
          <svg aria-hidden="true" className="w-7 h-7" style={{ color: "var(--color-settle-green-ink)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
          </svg>
        </div>
        <h2 className="font-serif text-2xl font-semibold text-stone-800 tracking-tight leading-snug text-balance">
          {allUsed ? "That's all for now" : "Done for now."}
        </h2>
        <p className="text-stone-600 text-base leading-relaxed text-pretty">
          {nothingHelped
            ? "Sometimes nothing clicks. Come back when you're ready."
            : "Come back whenever you need it."}
        </p>
        {nothingHelped && (
          <p
            className="text-sm text-stone-600 leading-relaxed rounded-lg px-3 py-2.5"
            style={{
              backgroundColor: "var(--color-care-bg)",
              border: "1px solid var(--color-care-border)",
            }}
          >
            If you&apos;re struggling, talking to someone you trust or a professional can help.{" "}
            <a
              href="https://findahelpline.com"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Support resources (opens in new tab)"
              className="underline underline-offset-2 transition-colors hover:opacity-80"
              style={{ color: "var(--color-care-ink)" }}
            >
              Support resources →
            </a>
          </p>
        )}
      </div>
      <button
        onClick={onHome}
        className="w-full py-4 rounded-2xl bg-stone-800 text-white text-base font-medium hover:bg-stone-700 active:scale-[0.98] transition-all mt-4"
      >
        Back home
      </button>
    </div>
  );
}
